...
在 0.1.2 语法补全的基础上，0.1.4 新增了以下语法（主要为 `.vraw` 静态语义与 `.cpps` Cstar 注解）。

### A. raw 静态语法（`.vraw` 文件）

由线性类型检查器 `linear.rs` 强制执行（0.1.4 从警告升级为硬错误）：

- **`ptr[T]`** — 指针类型，`T` 为指向对象。`ptr[T]` 是线性资源，必须在作用域退出前被消费（move / drop / dtor）。
- **`&T`** — 共享借用（不可变引用）。
- **`&mut T`** — 可变借用。
- **`u8` / `u16` / `u32` / `u64`** — 定宽无符号整数（不同于 0.1.2 的 `int`/`float` 动态类型）。
- **`Result[T, E]`** — 错误传播结果类型（成功 `Ok[T]` / 失败 `Err[E]`）。
- **`trait` / `impl` / `dtor` 块** — trait 声明、trait 实现、确定性析构器（作用域退出时由后端自动调用）。

### B. Cstar 注解（`.cpps` 文件）

在 0.1.2 `@pipeline` / `@patch` / `@isr_group` / `@prefetch` 骨架基础上，0.1.4 补全了实现，并新增：

- **`@pipeline`** — DMA 编排（多阶段数据搬运）。
- **`@patch`** — 固件差分更新（per-function 哈希）。
- **`@isr_group`** — 中断聚类 + WCET（最坏情况执行时间）估计。
- **`@prefetch`** — 缓存预取提示。
- **`@repo`** — 按尺寸的物理包管理。
- **`@section`** — 显式段/节放置（如 `@section(".isr_vector")`）。

### C. 底层语义关键字

- **`volatile`** — 访问标记为 volatile（禁止编译器优化读取/写入）。
- **`align(N)`** — 指定 `N` 字节对齐（如 `align(16) buf`）。
- **`asm { ... }` 块** — 内联汇编（0.1.2 raw 后端已支持；0.1.4 扩展到 ARM 后端，支持 AArch64 / ARM32 语法）。

---

## 0.1.2 语法补全

以下为 0.1.2 时期完成的 10 项语法补全（原文保留）。
...
Vredrs 0.1.4 语法补全完成。10 项 0.1.2 特性 + raw 静态语法 + Cstar 高级注解全部实现并通过验收。
