...
#[test]
fn test_type_generic_multi_param() {
    assert_parse_ok("fn, f[T, U](a: T, b: U): T\n    return, a\n/end");
}

#[test]
fn test_type_generic_with_constraint() {
    assert_parse_ok("fn, render[T: Drawable](obj: T)\n    return, 0\n/end");
}

#[test]
fn test_type_generic_multi_constraint() {
    assert_parse_ok("fn, f[T: Drawable + Comparable](x: T)\n    return, 0\n/end");
}

#[test]
fn test_generic_struct() {
    assert_parse_ok("struct, Box[T]\n    value, T\n/end");
}

#[test]
fn test_generic_struct_multi() {
    assert_parse_ok("struct, Pair[T, U]\n    first, T\n    second, U\n/end");
}

#[test]
fn test_generic_enum() {
    assert_parse_ok("enum, Option[T]\n    Some(T)\n    None\n/end");
}

#[test]
fn test_generic_enum_multi() {
    assert_parse_ok("enum, Result[T, E]\n    Ok(T)\n    Err(E)\n/end");
}
...
// ============================================================================
// .vraw mode syntax (Task 4-vraw-syntax)
// ============================================================================

#[test]
fn parses_volatile_set() {
    // `volatile set, x = 42` — volatile modifier on a set statement.
    // The parser should accept it and produce a normal assignment.
    use crate::parser::ast::{AssignOp, Stmt};
    let src = "volatile set, x = 42\n";
    let p = parse(src).expect("volatile set should parse");
    assert_eq!(p.declarations.len(), 1);
    match &p.declarations[0] {
        crate::parser::ast::TopLevel::Statement(Stmt::Assign(a)) => {
            assert_eq!(a.operator, AssignOp::Simple);
...
#[test]
fn parses_align_modifier_on_struct() {
    // `align(64) struct, Name ... /end` — align modifier prefix.
    assert_parse_ok("align(64) struct, DMA_Descriptor\n    addr, u64\n    length, u32\n/end\n");
}

#[test]
fn parses_align_modifier_on_set() {
    // `align(16) set, x = 7` — align modifier on a local declaration.
    assert_parse_ok("align(16) set, x = 7\n");
}

#[test]
fn parses_asm_with_gcc_constraints_comma_form() {
    // Comma form: asm, "template" : "out"(e) : "in"(e)
    let src = "asm, \"nop\" : \"=r\"(result) : \"r\"(a)\n";
    let p = parse(src).expect("asm with constraints should parse");
    assert_eq!(p.declarations.len(), 1);
    match &p.declarations[0] {
        crate::parser::ast::TopLevel::Statement(Stmt::Asm(a)) => {
            assert_eq!(a.template, "nop");
            assert_eq!(a.outputs.len(), 1);
            assert_eq!(a.outputs[0].constraint, "=r");
            assert_eq!(a.inputs.len(), 1);
            assert_eq!(a.inputs[0].constraint, "r");
        }
...
#[test]
fn parses_asm_brace_form_with_constraints() {
    // Brace form: asm { "template" : "out"(e) : "in"(e) }
    let src = "asm {\n    \"nop\"\n    : \"=r\"(result)\n    : \"r\"(a)\n}\n";
    let p = parse(src).expect("asm brace form should parse");
    assert_eq!(p.declarations.len(), 1);
    match &p.declarations[0] {
        crate::parser::ast::TopLevel::Statement(Stmt::Asm(a)) => {
            assert_eq!(a.template, "nop");
...
#[test]
fn parses_asm_without_constraints() {
    // Backwards compatibility: asm without constraints still parses.
    assert_parse_ok("asm, \"nop\"\n");
}

#[test]
fn parses_at_if_conditional_compile_true() {
    // @if (true) { ... } — should produce a ConditionalCompile node
    // selecting the then-body.
    use crate::parser::ast::TopLevel;
    let src = "@if (true) {\n    set, x = 1\n}\n";
    let p = parse(src).expect("@if should parse");
    assert_eq!(p.declarations.len(), 1);
    match &p.declarations[0] {
        TopLevel::ConditionalCompile(cc) => {
            assert_eq!(cc.then_body.len(), 1);
            assert!(cc.else_body.is_none());
        }
        other => panic!("expected ConditionalCompile, got {:?}", other),
    }
}

#[test]
fn parses_at_if_with_else() {
    // @if (cond) { ... } @else { ... } — should produce a
    // ConditionalCompile with both then_body and else_body populated.
    use crate::parser::ast::TopLevel;
    let src = "@if (true) {\n    set, x = 1\n}\n@else {\n    set, x = 2\n}\n";
    let p = parse(src).expect("@if @else should parse");
    assert_eq!(p.declarations.len(), 1);
    match &p.declarations[0] {
        TopLevel::ConditionalCompile(cc) => {
            assert_eq!(cc.then_body.len(), 1);
            assert!(cc.else_body.is_some());
            assert_eq!(cc.else_body.as_ref().unwrap().len(), 1);
        }
        other => panic!("expected ConditionalCompile, got {:?}", other),
    }
}
...
#[test]
fn parses_at_if_keyword_if_form() {
    // `if` is a keyword in Vredrs; the @if parser must accept it.
    use crate::parser::ast::TopLevel;
    let src = "@if (false) {\n    set, x = 1\n}\n";
    let p = parse(src).expect("@if with keyword `if` should parse");
    match &p.declarations[0] {
        TopLevel::ConditionalCompile(_) => {}
        other => panic!("expected ConditionalCompile, got {:?}", other),
    }
}

#[test]
fn parses_dtor_block() {
    // `dtor, Type ... /end` — destructor block.
    use crate::parser::ast::TopLevel;
    let src = "dtor, Resource\n    set, freed = 1\n/end\n";
    let p = parse(src).expect("dtor should parse");
    match &p.declarations[0] {
        TopLevel::DtorBlock(dtor) => {
            assert_eq!(dtor.type_name.name, "Resource");
            assert_eq!(dtor.body.len(), 1);
        }
        other => panic!("expected DtorBlock, got {:?}", other),
    }
}

#[test]
fn parses_ptr_atomics_method_calls() {
    // `p.load_acquire()` and `p.store_release(v)` should parse as
    // method calls (Call with MemberAccess callee).
    use crate::parser::ast::{Expr, Stmt};
    let src = "set, v = p.load_acquire()\nq.store_release(42)\n";
    let p = parse(src).expect("ptr atomics should parse");
    assert_eq!(p.declarations.len(), 2);
    // First: set, v = p.load_acquire()
    match &p.declarations[0] {
        crate::parser::ast::TopLevel::Statement(Stmt::Assign(a)) => {
            match &a.value {
                Expr::Call(c) => {
                    match c.callee.as_ref() {
                        Expr::MemberAccess(m) => {
                            assert_eq!(m.member.name, "load_acquire");
                        }
