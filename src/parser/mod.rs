    ...
    fn parse_fn_params(&mut self) -> Result<Vec<FnParam>> {
        ...
        if self.current_kind() == TokenKind::LBracket {
            ...
            while self.current_kind() != TokenKind::RBracket && !self.is_at_end() {
                self.advance(); // skip type params for now
                if self.current_kind() == TokenKind::Comma {
                    self.advance();
                }
            }
    ...
    fn parse_prec(&mut self, min: usize) -> Result<Expr> {
        ...
        loop {
            ...
            match self.current_kind() {
                ...
                TokenKind::QuestionDot => {
                    self.advance();
                    left = Expr::OptionalChain(OptionalChainExpr {
                        target: Box::new(left),
                        chain: vec![],
                        span: Span::dummy(),
                    });
...
fn prec(k: &TokenKind) -> usize {
    match k {
        TokenKind::DotDot | TokenKind::DotDotDot => 0, // handled by range logic, but returned with high precedence so it always matches > min
        TokenKind::As => 1,
        TokenKind::Question => 1,
        TokenKind::PipeArrow => 2,
        TokenKind::QuestionQuestion => 3,
        TokenKind::And | TokenKind::Or => 4,
        TokenKind::Eq
        | TokenKind::Ne
        | TokenKind::Lt
        | TokenKind::Gt
