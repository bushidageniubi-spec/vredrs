...
impl Parser {
    ...
    fn parse_top_level(&mut self) -> Result<TopLevel> {
        ...
        match self.current_kind() {
            ...
            TokenKind::Unsafe => Ok(TopLevel::Statement(Stmt::UnsafeBlock(self.parse_unsafe()?))),
    ...
    fn parse_for(&mut self) -> Result<Stmt> {
        ...
        if self.current_kind() == TokenKind::In {
            ...
            // Optional `step` clause for range iterables: `for, i, in, 1..10 step 2`.
            // This is rarely used (the range literal already carries a step),
            // but we accept it for completeness and store it on the ForIn
            // iterable as a Range with a step. Since the iterable is already
            // parsed, we just ignore a trailing `step` here — the range
            // literal `1..10 step 2` is handled in parse_prec.
            let b = self.parse_block()?;
    ...
    fn parse_prefix(&mut self) -> Result<Expr> {
        match self.current_kind() {
            ...
            TokenKind::Channel | TokenKind::Send | TokenKind::Receive | TokenKind::Identifier => {
                let id = self.parse_id()?;
                if self.current_kind() == TokenKind::LParen {
                    self.advance();
                    let a = self.parse_args()?;
