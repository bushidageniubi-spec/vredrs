...
pub struct Parser {
    ...
    /// Side-channel for generic type-parameter names in declaration order.
    /// Populated alongside `pending_type_constraints` by `parse_fn_params`
    /// and `parse_type_param_list`.
...
impl Parser {
    ...
    fn parse_top_level(&mut self) -> Result<TopLevel> {
        // @if(...) — conditional compilation directive. We detect this
        // *before* `parse_annotations` (which would otherwise attach it
        // to the following declaration as an `Annotation{name:"if"}`).
        // Note: `if` is a keyword (TokenKind::If), not an Identifier,
        // so we look for either form.
        if self.current_kind() == TokenKind::At
            && (self.peek_kind() == Some(TokenKind::Identifier)
                || self.peek_kind() == Some(TokenKind::If))
            && self.peek_lex() == "if"
        {
            return self.parse_conditional_compile();
        }
        // `align(N)` modifier prefix — raw-mode hint. We capture it as
        // an `@align(N)` annotation on the following declaration so the
        // raw backends can honor it (emit `.balign N` for struct
        // globals, pad field offsets, etc.). The annotation is prepended
        // to any `@...` annotations that follow.
        let mut annotations = Vec::new();
        if let Some(a) = self.consume_align_modifier() {
            annotations.push(a);
        }
        annotations.extend(self.parse_annotations());
        // The modifier can also appear after annotations (rare).
        if let Some(a) = self.consume_align_modifier() {
            annotations.push(a);
        }
    ...
    /// Consume an optional `align(N)` modifier prefix. The integer `N`
    /// is captured as an `@align(N)` annotation so the raw backends can
    /// honor it (emit `.balign N` for struct globals, pad field offsets
    /// to the alignment boundary, etc.). Both `align (N)` and `align(N)`
    /// spellings are accepted. Returns `Some(Annotation)` when an
    /// `align(N)` modifier was present and consumed.
    fn consume_align_modifier(&mut self) -> Option<Annotation> {
        if self.current_kind() == TokenKind::Identifier
            && self.lex() == "align"
            && (self.peek_kind() == Some(TokenKind::LParen)
                || (self.peek_kind() == Some(TokenKind::Newline)
                    && self.tokens.get(self.pos + 2).map(|t| t.kind == TokenKind::LParen).unwrap_or(false)))
        {
            self.advance(); // consume `align`
            self.skip_newlines();
            if self.current_kind() == TokenKind::LParen {
                self.advance();
                // Parse the alignment value (an integer or an expression)
                // and capture it as an `@align(N)` annotation argument.
                let mut args: Vec<AnnotationArg> = Vec::new();
                if !self.is_at_end() && self.current_kind() != TokenKind::RParen {
    ...
    /// Parse a generic type-parameter list `[T, U: Constraint, V: A + B]`.
    /// Populates `pending_type_constraints` and `pending_type_params`.
    /// Used by `parse_fn_params` and `parse_struct_def`/`parse_enum_def`.
    fn parse_type_param_list(&mut self) -> Result<()> {
        ...
        while self.current_kind() != TokenKind::RBracket && !self.is_at_end() {
            ...
            if self.current_kind() == TokenKind::Identifier {
                ...
                self.pending_type_constraints.insert(tp_name.name.clone(), constraints);
                self.pending_type_params.push(tp_name.name);
            }
            if self.current_kind() == TokenKind::Comma {
                self.advance();
                continue;
    ...
    fn parse_struct_def(&mut self) -> Result<StructDef> {
        ...
        self.expect(TokenKind::Comma)?;
        let name = self.parse_id()?;
        // Optional generic type-parameter list: `struct, Box[T] { ... }`
        let (type_params, type_constraints) = if self.current_kind() == TokenKind::LBracket {
            self.parse_type_param_list()?;
            let tp = std::mem::take(&mut self.pending_type_params);
            let tc = std::mem::take(&mut self.pending_type_constraints);
            (tp, tc)
        } else {
    ...
    fn parse_class_def(&mut self) -> Result<ClassDef> {
        ...
        let mut methods = vec![];
        // Class body can be:
        //   1. Indented block terminated by Dedent or /end
        //   2. Single-line (no indent) — rare
        if self.current_kind() == TokenKind::Indent {
            self.advance();
            loop {
                self.skip_newlines();
                // End of class body: Dedent (indentation ended) or /end
                // directive.  We must be careful: parse_fn_def's
                // parse_block already consumed the *method's* Dedent and
                // /end, so any Dedent we see here belongs to the class
                // block itself.
                if self.current_kind() == TokenKind::Dedent || self.is_at_end() {
                    // Consume the class-body Dedent.
                    if self.current_kind() == TokenKind::Dedent {
                        self.advance();
                    }
                    // After the Dedent, there may be a /end directive
                    // for the class block.
                    self.skip_newlines();
                    if self.current_kind() == TokenKind::DirectiveOrScope && self.lex() == "/end" {
                        self.advance();
                ...
                if self.current_kind() == TokenKind::Fn || self.current_kind() == TokenKind::Async {
                    ...
                    // Field declaration: may be "name" or "name, type"
                    // or "name1, name2, name3" (multiple fields on one
                    // line, comma-separated).
                    loop {
                        let id = self.parse_id()?;
                        // Optional type annotation: "name, Type"
                        let ta = if self.current_kind() == TokenKind::Comma {
                            self.advance(); // consume comma
                            if self.current_kind() == TokenKind::Identifier {
                                let next_lex = self.lex().to_string();
                                if next_lex.chars().next().map(|c| c.is_uppercase()).unwrap_or(false) {
                                    Some(self.parse_type_expr()?)
                                } else {
                                    // It's another field name — push
                                    // current field and continue loop.
                        ...
                        // Check for more comma-separated fields.
                        if self.current_kind() == TokenKind::Comma {
                            self.advance();
                            if self.current_kind() == TokenKind::Identifier {
                                continue;
                    ...
                    // Unexpected token — consume it to avoid infinite
                    // loop and break out.
                    self.advance();
                    break;
                }
        ...
        // Consume any trailing /end for the class block (idempotent —
        // may have already been consumed above).
        self.skip_newlines();
        if self.current_kind() == TokenKind::DirectiveOrScope && self.lex() == "/end" {
            self.advance();
        }
    ...
    fn parse_enum_def(&mut self) -> Result<EnumDef> {
        ...
        self.expect(TokenKind::Comma)?;
        let name = self.parse_id()?;
        // Optional generic type-parameter list: `enum, Option[T] { ... }`
        let (type_params, type_constraints) = if self.current_kind() == TokenKind::LBracket {
            self.parse_type_param_list()?;
            let tp = std::mem::take(&mut self.pending_type_params);
            let tc = std::mem::take(&mut self.pending_type_constraints);
    ...
    /// Parse an `@if (cond) { ... } else { ... }` conditional-compilation
    /// directive into a `TopLevel::ConditionalCompile` node. The `@if`
    /// part has already been detected by `parse_top_level`; this method
    /// assumes the current token is `@`.
    ///
    /// The body is a sequence of top-level declarations, terminated by
    /// either a `}` (brace form) or `/end` / dedent (indented form).
    /// The optional `@else` introduces an alternative body.
    fn parse_conditional_compile(&mut self) -> Result<TopLevel> {
        self.expect(TokenKind::At)?;
        // `if` is a keyword (TokenKind::If) in Vredrs; accept either
        // the keyword or a bare identifier named "if".
            ...
            && self.lex() == "if"
        {
            ...
            return Err(self.err("Expected 'if' after @"));
        }
        // Optional leading comma (Vredrs style: `@if, cond ...`).
        self.eat_comma();
        self.expect(TokenKind::LParen)?;
        let condition = self.parse_expr()?;
        self.expect(TokenKind::RParen)?;
        self.skip_newlines();
        // Optional comma between condition and body.
        self.eat_comma();
        let then_body = self.parse_top_level_body()?;
        // Optional @else { ... }
        let mut else_body = None;
        let saved_pos = self.pos;
        self.skip_newlines();
        if self.current_kind() == TokenKind::At
            && (self.peek_kind() == Some(TokenKind::Else)
                || self.peek_kind() == Some(TokenKind::Identifier))
            && self.peek_lex() == "else"
        {
            ...
            self.skip_newlines();
            self.eat_comma();
            else_body = Some(self.parse_top_level_body()?);
        } else {
            // Not an @else — rewind to the saved position so the next
            // call to parse_top_level can re-examine the token.
            self.pos = saved_pos;
        }
        Ok(TopLevel::ConditionalCompile(ConditionalCompile {
            condition,
            then_body,
            else_body,
            span: Span::dummy(),
    ...
    /// Parse a block of top-level declarations for `@if` / `@else`.
    /// Two forms are accepted:
    ///   * `{ decl... }` — brace-delimited block (raw-mode style).
    ///   * Indented block terminated by `/end` or dedent.
    fn parse_top_level_body(&mut self) -> Result<Vec<TopLevel>> {
        ...
        // Indented block: optional Indent token, then declarations,
        // then either /end or dedent.
        if self.current_kind() == TokenKind::Indent {
            self.advance();
            loop {
                self.skip_newlines();
            ...
            // Single-declaration body (no indent, no braces).
            if !self.is_at_nl() && !self.is_at_end() {
                body.push(self.parse_top_level()?);
            }
        }
    ...
    /// Parse optional GCC-style inline-asm operand lists following the
    /// template string. The grammar is:
    ///
    /// ```text
    /// : "constraint"(expr), "constraint"(expr)   // outputs
    /// : "constraint"(expr), "constraint"(expr)   // inputs
    /// ```
    ///
    /// Either side may be empty (a bare `:` introduces an empty list).
    /// Returns `(outputs, inputs)`. Operands that fail to parse are
    /// silently skipped (the asm block still produces a valid AST).
    ///
    /// Indentation tokens emitted by the lexer (because operand lines
    /// are typically more indented than the `asm` keyword) are skipped
    /// between operands. The matching `Dedent` is left for the
    /// enclosing block parser to consume.
    fn parse_asm_operands(&mut self) -> Result<(Vec<AsmOperand>, Vec<AsmOperand>)> {
        let mut outputs = Vec::new();
        let mut inputs = Vec::new();
        // Helper: skip newlines, comments, and `Indent` tokens that
        // appear inside the operand list (the lexer emits them when
        // operand lines are more indented than the `asm` keyword).
        // `Dedent` is left alone so the enclosing block parser can
        // match its `Indent`.
        let skip_ws = |p: &mut Parser| {
            loop {
                match p.current_kind() {
                    ...
                    | TokenKind::Indent => p.advance(),
                    _ => break,
                }
            }
        };
        // First `:` introduces output operands.
        skip_ws(self);
        if self.current_kind() == TokenKind::Colon {
            self.advance();
            skip_ws(self);
            loop {
                ...
                if self.current_kind() == TokenKind::LParen {
                    self.advance();
                    let expr = self.parse_expr().unwrap_or(Expr::Null(NullLiteral {
                        span: Span::dummy(),
                    }));
                    if self.current_kind() == TokenKind::RParen {
                        self.advance();
                    ...
                    outputs.push(AsmOperand {
                        constraint,
                        expr: Expr::Null(NullLiteral { span: Span::dummy() }),
                        span: Span::dummy(),
                    });
                }
                if self.current_kind() == TokenKind::Comma {
        ...
        // Second `:` introduces input operands.
        skip_ws(self);
        if self.current_kind() == TokenKind::Colon {
            self.advance();
            skip_ws(self);
        ...
        // Clobber list (a third `:`) — parse and discard.
        skip_ws(self);
        if self.current_kind() == TokenKind::Colon {
            self.advance();
            while self.current_kind() == TokenKind::StringLiteral {
                self.advance();
                if self.current_kind() == TokenKind::Comma {
                    self.advance();
                } else {
    ...
    fn parse_stmt(&mut self) -> Result<Stmt> {
        ...
        // `align(N)` modifier prefix on a local declaration (e.g.
        // `align(64) set, x = ...`). We parse and capture as an
        // `@align(N)` annotation, but AssignStmt has no annotations
        // field, so the captured annotation is discarded here. (The
        // local-alignment hint is currently only needed for struct
        // globals, which are handled in parse_top_level.) The modifier
        // must parse without error so `align(N) set, ...` is accepted.
        let _ = self.consume_align_modifier();
        // `volatile set, x = ...` — raw-mode hint that this assignment
        // must not be optimized away. `volatile` lexes as an
        // Identifier (no dedicated keyword), so we detect it here and
        // delegate to the normal `set` path.
        if self.current_kind() == TokenKind::Identifier
            && self.lex() == "volatile"
            && self.peek_kind() == Some(TokenKind::Set)
        {
            self.advance(); // consume `volatile`
            self.advance(); // consume `set`
            // Emit the assignment as a normal Set; the volatility hint
            // is recorded by the backend as a comment (no AST flag is
            // needed since current backends don't optimize away
            // reads/writes anyway).
        ...
        match self.current_kind() {
            ...
            TokenKind::Identifier => {
                ...
                if id.name == "asm" {
                    ...
                    //   1. `asm, "template" : "out"(e) : "in"(e)` (comma form)
                    //   2. `asm { "template" : "out"(e) : "in"(e) }` (brace form)
                    let (template, outputs, inputs) = if self.current_kind() == TokenKind::LBrace {
                        self.advance();
                        self.skip_newlines();
                        // Concatenate consecutive template string literals.
                        let mut tmpl = String::new();
                        while self.current_kind() == TokenKind::StringLiteral {
                            tmpl.push_str(&self.expect_str()?);
                            self.skip_newlines();
                        }
                        let (outs, ins) = self.parse_asm_operands()?;
                        self.skip_newlines();
                        if self.current_kind() == TokenKind::RBrace {
                            self.advance();
                        }
                ...
                // Resume precedence-climbing so that binary / postfix /
                // range / cast / null-coalesce / pipe operators following
                // the identifier are folded into the same expression
                // (e.g. `x + x`, `x * 2`, `x |> f`, `a..b`). Without
                // this, a bare `x + x` statement would be split into
                // three separate statements (`x`, `+`, `x`), which
                // silently breaks implicit-return function bodies like
                // `fn, double(x) x * 2 /end`.
                let expr = self.parse_prec_continue(Expr::Identifier(id), 0)?;
                self.nl();
                Ok(Stmt::Expr(ExprStmt {
                    expr,
                    span: Span::dummy(),
    ...
    fn parse_prec(&mut self, min: usize) -> Result<Expr> {
        ...
        self.parse_prec_continue(left, min)
    }

    /// Continue parsing a precedence-climbing expression starting from
    /// the given `left` operand. This is the binary/postfix operator
    /// loop, factored out of `parse_prec` so that callers which have
    /// already consumed a leading identifier (e.g. `parse_stmt`'s
    /// Identifier arm) can resume parsing binary expressions like
    /// `x + x` instead of treating the identifier as a complete
    /// expression.
    fn parse_prec_continue(&mut self, mut left: Expr, min: usize) -> Result<Expr> {
        loop {
            if self.is_at_nl() {
                break;
            }
    ...
    fn parse_prefix(&mut self) -> Result<Expr> {
        match self.current_kind() {
            TokenKind::IntegerLiteral => {
                let t = self.adv_tok();
                let value = match parse_i64_literal(&t.lexeme) {
                    Some(v) => v,
                    None => {
                        // Distinguish overflow (all-digits body) from
                        // invalid format. Either way this is a hard parse
                        // error — silently returning 0 would mask bugs.
                        let cleaned: String =
                            t.lexeme.chars().filter(|c| *c != '_').collect();
                        let body = cleaned
                            .strip_prefix('-')
                            .map(str::to_string)
                        ...
                        let core = body
                            ...
                            .or_else(|| body.strip_prefix("0b")).or_else(|| body.strip_prefix("0B"))
                            .or_else(|| body.strip_prefix("0o")).or_else(|| body.strip_prefix("0O"))
                            .map(str::to_string)
                            .unwrap_or(body);
                        let is_all_digits = !core.is_empty()
                        ...
                        if is_all_digits {
                            return Err(CompilerError::parse_error(
                                format!(
                                    "integer literal overflow: {} exceeds i64 range",
                                    t.lexeme
                                ),
                                t.span,
                            ));
                        ...
                        return Err(CompilerError::parse_error(
                            format!("invalid integer literal: {}", t.lexeme),
                            t.span,
                        ));
                    }
                };
            ...
            TokenKind::FloatLiteral => {
                let t = self.adv_tok();
                let value: f64 = match t.lexeme.parse::<f64>() {
                    Ok(v) if v.is_finite() => v,
                    Ok(_) => {
                        // +inf / -inf / NaN from a literal — the lexeme
                        // must be unreasonably large or malformed.
                        return Err(CompilerError::parse_error(
                            format!(
                                "float literal out of range: {} is not a finite f64",
                                t.lexeme
                            ),
                            t.span,
                        ));
                    ...
                    Err(_) => {
                        return Err(CompilerError::parse_error(
                            format!("invalid float literal: {}", t.lexeme),
                            t.span,
                        ));
                    }
                };
            ...
            TokenKind::Fn => {
                self.advance();
                let p = self.parse_fn_params()?;
                // Lambda body: try expression first.
                // If the expression is followed by an assignment operator
                // (+=, -=, etc.), it's a compound assignment used as a
                // lambda body — wrap it in AssignExpr.
                let b = match self.parse_expr() {
                    Ok(e) => {
                        // Check if this is `identifier op= value` —
                        // parse_expr consumed the identifier, now check
                        ...
                        if self.is_assign_token() {
                            ...
                            match self.parse_expr() {
                                Ok(rhs) => {
                                    // Construct an AssignStmt from the
                                    // expression (must be an identifier or
                                    // indexable expression).
                                    let target = match &e {
                                        Expr::Identifier(id) => {
                                            Assignee::Identifier(id.clone())
                                        }
                                        Expr::Index(ix) => {
                                            Assignee::Index(ix.clone())
                                        }
                                        Expr::MemberAccess(m) => {
                                            Assignee::Member(m.clone())
                                        }
                                        _ => Assignee::Identifier(Identifier {
                                            name: "_".to_string(),
                                            span: Span::dummy(),
                                        }),
                                    };
                                    Expr::AssignExpr(Box::new(AssignStmt {
                                        targets: vec![target],
                                        value: rhs,
                                        operator: op,
                                        span: Span::dummy(),
                    ...
                    Err(_) => {
                        ...
                        if let Ok(stmt) = self.parse_stmt() {
                            match stmt {
                                ...
                                Stmt::Assign(a) => {
                                    // Compound/simple assignment as lambda body:
                                    // wrap in Expr::AssignExpr which executes
                                    // the assignment and returns the new value.
                                    Expr::AssignExpr(Box::new(a))
                                }
                                _ => Expr::Null(NullLiteral {
                                    span: Span::dummy(),
                                }),
    ...
    fn parse_type_expr(&mut self) -> Result<TypeExpr> {
        match self.current_kind() {
            TokenKind::BitAnd => {
                ...
                // Parse optional lifetime annotation: &'a T, &'a mut T, &'static T
    ...
    /// Lexeme of the *next* token (one past the current position), or
    /// empty string when there is no next token. Used to look ahead at
    /// an identifier's text without consuming it.
    fn peek_lex(&self) -> &str {
        if self.pos + 1 < self.tokens.len() {
            &self.tokens[self.pos + 1].lexeme
        } else {
            ""
        }
    }
