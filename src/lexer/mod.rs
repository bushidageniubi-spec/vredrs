...
impl Lexer {
    ...
    pub fn tokenize(&mut self) -> Result<Vec<Token>> {
        ...
        while self.pos < len {
            ...
            match ch {
                ...
                '\'' => {
                    // Check if this is a lifetime ('a, 'b, 'static)
                    // or a character/string literal.
                    // Bug V1 fix: 'hello' (多字符) 应该是字符串，不是生命周期。
                    // 生命周期只在 ' 后跟单个字母 + 非字母数字时才有效。
                    let peek = self.peek_char();
                    if peek.is_alphabetic() {
                        // 看后面是否还有更多字母数字（多字符 → 字符串）
                        let pos = self.pos;
                        self.advance();
    ...
    fn generate_dedents(&mut self, target: usize) -> Result<()> {
        ...
        if *self.indent_stack.last().unwrap() != target {
            ...
            return Err(CompilerError::lex_error(
                format!(
                    "indentation mismatch: this line has {} spaces but the enclosing block expects {} spaces.\n\
                     hint: align '/end' with the opening keyword (e.g. 'if' or 'fn'), not with the body.",
                    target,
    ...
    fn scan_hash_or_comment(&mut self) -> Result<()> {
        ...
        // Heuristic: if # is preceded by an identifier/number/]/) char
        // (no space before it), it's the postfix # operator even at EOL.
        // If preceded by space or at line start, it's a comment.
        let prev_char = if start > 0 {
            self.source[start - 1]
        } else {
            ' '
        };
        let is_after_expr = prev_char.is_alphanumeric()
            || prev_char == '_'
            || prev_char == ']'
            || prev_char == ')'
            || prev_char == '"';
        if is_after_expr && self.pos < self.source.len() {
            // # directly after an expression character → postfix operator
            self.add_token(TokenKind::Hash, start, self.pos, "#");
            return Ok(());
        }
        if self.pos >= self.source.len() {
            // # at EOF → postfix operator (if after expr) or comment
            if is_after_expr {
                self.add_token(TokenKind::Hash, start, self.pos, "#");
            }
            return Ok(());
    ...
    fn scan_number(&mut self) -> Result<()> {
        ...
        if self.current_char() == '.' {
            // Don't consume '.' if the next char is also '.', which
            // means this is a range operator (.. or ...).
            let peek = self.peek_char();
            if peek == '.' {
                // Range operator — don't consume '.', leave it for the
                // range token.
            } else {
                // Consume '.' as part of a float literal.
                // "0." → 0.0, "3.14" → 3.14
                is_float = true;
    ...
    /// Scan a lifetime token: `'a`, `'b`, `'static`, etc.
    /// The `'` has already been consumed by the caller. We scan the
    /// identifier part and emit a `TokenKind::Lifetime(name)`.
    fn scan_lifetime(&mut self) {
        let start = self.pos - 1; // Include the '
        // Scan the identifier part (a-z, A-Z, 0-9, _).
        ...
        self.add_token(TokenKind::Lifetime(name), start, self.pos, "");
    }
