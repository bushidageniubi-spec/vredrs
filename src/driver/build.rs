...
pub fn build_project(project_dir: &str, opts: &BuildOptions) -> Result<BuildResult> {
    ...
    let executable: Option<PathBuf> = if !cached_objs.is_empty() {
        ...
        eprintln!("[vredrs]   linking {} object files → {}", cached_objs.len(), exe.display());
        match link_objects(&cached_objs, &exe, opts) {
            Ok(()) => {
                eprintln!("[vredrs]   linked: {}", exe.display());
                Some(exe)
            },
            Err(e) => { eprintln!("[vredrs]   warning: link failed: {}", e); None }
        }
    } else { None };

    if let Some(ref fw) = firmware_path {
        eprintln!("[vredrs]   firmware: {}", fw.display());
    }

    let elapsed = start.elapsed().as_millis();
    eprintln!("[vredrs] build complete: {} compiled, {} cached, {} ms",
...
fn compile_veds_to_obj(source: &Path, output: &Path, release: bool, debug: bool) -> std::result::Result<(), String> {
    ...
    let mut gen = crate::codegen::llvm_full::FullLlvmGen::new();
    let ir = gen.generate(&program).map_err(|e| e.message().to_string())?;
    ...
    match std::process::Command::new("clang").args(&clang_args).output() {
        ...
        Ok(out) => Err(format!("clang: {}", String::from_utf8_lossy(&out.stderr))),
        Err(_) => { eprintln!("[vredrs]   clang not found, leaving .ll"); Ok(()) }
    }
}
...
fn compile_vraw_to_obj(source: &Path, output: &Path, target_arch: &str, _release: bool, _debug: bool) -> std::result::Result<(), String> {
    ...
    let mut parser = crate::parser::Parser::new(tokens, 0);
    let program = parser.parse_program().map_err(|e| e.message().to_string())?;
    // Route to the correct architecture backend
    match target_arch {
        "x86_64" => crate::codegen::cstar::raw::compile_to_x86_assembly(&program, output).map_err(|e| e.to_string()),
        "aarch64" | "arm" => crate::codegen::cstar::raw::compile_to_arm_assembly(&program, output, target_arch).map_err(|e| e.to_string()),
        _ => crate::codegen::cstar::raw::compile_to_x86_assembly(&program, output).map_err(|e| e.to_string()),
    }
}
