...
pub fn collect_modules(
    ...
    // Use the content hash as the dedup key, but also track the source
    // path to avoid deduplicating different files with identical content.
    // We append a counter to make each entry unique even for identical
    // sources from different paths.
    let dedup_key = hash.clone();
    if !seen.insert(dedup_key) {
        return Ok(());
    }

    // Process imports FIRST (dependency order: imports before importers).
    ...
    // Push self AFTER imports (dependency order).
...
pub fn resolve_import_path(base_dir: &Path, module: &str) -> Result<PathBuf> {
    ...
    // Try relative to base_dir, then bundled stdlib paths.
    let candidates = [
        base_dir.join(&raw),
        base_dir.join(format!("{}.veds", raw)),
        base_dir.join(format!("{}.veds", dotted)),
        // Bundled stdlib: src/runtime/std/
        PathBuf::from("src/runtime/std").join(format!("{}.veds", raw)),
        PathBuf::from("../src/runtime/std").join(format!("{}.veds", raw)),
        // Also try absolute paths relative to the vredrs executable
        PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("src/runtime/std").join(format!("{}.veds", raw)),
    ];
