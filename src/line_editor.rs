...
/// Built-in function names available for Tab completion in the REPL.
const BUILTIN_NAMES: &[&str] = &[
    ...
    "if", "for", "while", "fn", "class", "return", "break",
    "continue", "throw", "try", "catch", "with", "match", "import",
    "export", "set", "del", "defer", "yield", "spawn", "loop",
];
...
impl LineEditor {
    ...
    /// empty line (EOF). The returned line does not include the trailing
    /// newline. When stdin is not a TTY (e.g. piped input), falls back to
    /// simple line-buffered reads so the escape sequences used for
    /// redrawing don't pollute the output.
    pub fn read_line(&mut self, prompt: &str) -> Option<String> {
        ...
        let result = self.read_line_inner(&mut stdout);
    ...
    fn read_line_inner(&mut self, stdout: &mut impl Write) -> Option<String> {
        ...
        loop {
            ...
            match c {
                ...
                // Tab (0x09) — completion.
                0x09 => {
                    // Attempt completion using the built-in candidate list.
    ...
    /// clear to end of line, write the buffer, then position the cursor.
    fn redraw(&self, stdout: &mut impl Write, buf: &str, cursor: usize) {
        // \r moves to column 0; \x1B[K clears to end of line.
        let _ = write!(stdout, "\r\x1B[K{}", buf);
