    ...
    fn gen_call_with_args(
        ...
        let func_name = match c.callee.as_ref() {
            ...
            other => {
                ...
                if callee.ty == Ty::Fn {
                    // Indirect call: cast i64 fn-ptr to the uniform closure
                    // signature `%vredrs.value (%vredrs.value, ..., ptr)*`
                    // and call with each arg wrapped as %vredrs.value.
                    ...
                    self.buf.push_str(&format!(
                        "  {} = inttoptr i64 {} to {}\n",
                        fn_ptr_typed, callee.repr, fn_ty
                    ));
                    ...
                    // Pass null env pointer (no captures in 1.0).
                    call_args.push("ptr null".to_string());
