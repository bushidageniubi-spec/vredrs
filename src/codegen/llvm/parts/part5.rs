    ...
    fn gen_expr(&mut self, e: &Expr, vars: &mut HashMap<String, LocalSlot>) -> Result<Val> {
        match e {
            ...
            Expr::NullCoalesce(nc) => {
                // `left ?? right` — proper null-coalescing.
                //
                // Evaluate `left` into a temporary, then check whether it is
                // null. We do this by wrapping `left` into a tagged
                // `%vredrs.value = { i8 tag, i64 payload }` and reading the
                // first element (the tag). The null tag is 0
                // (`vredrs_value_nil()`). If `tag == 0`, the left side is null
                // and we evaluate the right side; otherwise we return left.
                //
                // Pointer types (Str/Obj/List/...) in this backend represent
                // `null` as a null pointer rather than as a tagged nil, so we
                // additionally OR in a pointer-null check for those types —
                // this lets `obj ?? default` and `str ?? default` work even
                // though the wrapped tag for those types is non-zero
                // (e.g. tag=8 for an obj that happens to be a null pointer).
                // The two paths are merged with an LLVM `phi`.
                let l_raw = self.gen_expr(&nc.left, vars)?;
                let l_ty = l_raw.ty.clone();
                // Wrap into %vredrs.value so we can read its tag. For
                // `Ty::Value` this is a no-op; for pointer types it builds a
                // tagged value with the type's tag (e.g. tag=8 for obj); for
                // `Ty::Void`/`Ty::Ptr` it returns the value unchanged (these
                // are never null in practice).
                let l = self.wrap_value(l_raw.clone())?;
                // Extract the tag (first element of `{ i8, i64 }`).
                let tag = self.new_var();
                self.buf.push_str(&format!(
                    "  {} = extractvalue %vredrs.value {}, 0\n",
                    tag, l.repr
                ));
                // `icmp eq i8 tag, 0` → is_null (catches tagged nil).
                let is_null_tag = self.new_var();
                self.buf.push_str(&format!(
                    "  {} = icmp eq i8 {}, 0\n",
                    is_null_tag, tag
                ));
                // For pointer types, also check the underlying pointer
                // against null. This catches `null` (which is represented as
                // a null pointer for Obj/Str/List/...).
                let is_null = match l_ty {
                    Ty::Str
                    | Ty::List
                    | Ty::Dict
                    | Ty::Set
                    ...
                    | Ty::Coro
                    | Ty::Ptr => {
                        let is_null_ptr = self.new_var();
                        self.buf.push_str(&format!(
                            "  {} = icmp eq {} {}, null\n",
                            is_null_ptr, l_ty.ir(), l_raw.repr
                        ));
                        let combined = self.new_var();
                        self.buf.push_str(&format!(
                            "  {} = or i1 {}, {}\n",
                            combined, is_null_tag, is_null_ptr
                        ));
                        combined
                    }
                    _ => is_null_tag,
                };
                let left_lbl = self.new_lbl();   // return left
                let right_lbl = self.new_lbl();  // evaluate right
                let end_lbl = self.new_lbl();    // merge
                // If null, go evaluate right; otherwise, go to the left
                // (return-left) block.
                self.buf.push_str(&format!(
                    "  br i1 {}, label %L{}, label %L{}\n",
                    is_null, right_lbl, left_lbl
                ));
                // Left branch: just pass through the wrapped left value.
                self.buf.push_str(&format!("L{}:\n", left_lbl));
                self.buf
                    .push_str(&format!("  br label %L{}\n", end_lbl));
                // Right branch: evaluate right and wrap it into a
                // %vredrs.value so the phi has a uniform type.
                self.buf.push_str(&format!("L{}:\n", right_lbl));
                let r_raw = self.gen_expr(&nc.right, vars)?;
                let r = self.wrap_value(r_raw)?;
                self.buf
                    .push_str(&format!("  br label %L{}\n", end_lbl));
                // Merge: phi the wrapped values from both branches.
                self.buf.push_str(&format!("L{}:\n", end_lbl));
                let res = self.new_var();
                self.buf.push_str(&format!(
                    "  {} = phi %vredrs.value [ {}, %L{} ], [ {}, %L{} ]\n",
                    res, l.repr, left_lbl, r.repr, right_lbl
                ));
                Ok(Val::new(Ty::Value, res))
            }
            Expr::Qualified(q) => {
                // module.symbol → resolve to builtin name "module_symbol"
                if q.parts.len() >= 2 {
                    let mod_name = &q.parts[0].name;
                    let sym_name = &q.parts[1].name;
                    let builtin_name = format!("{}_{}", mod_name, sym_name);
                    if self.builtins.contains(builtin_name.as_str()) {
                        // Call the builtin with no args (it's a value reference)
                        let mut empty_args = Vec::new();
                        return self.gen_builtin_call(&builtin_name, empty_args, vars);
                    }
                    // For 3+ parts: module.sub.symbol → module_sub_symbol
                    if q.parts.len() >= 3 {
                        let full = q.parts.iter()
                            .map(|p| p.name.clone())
                            .collect::<Vec<_>>()
                            .join("_");
                        if self.builtins.contains(full.as_str()) {
                            return self.gen_builtin_call(&full, Vec::new(), vars);
                        }
                    }
                }
                Err(CompilerError::codegen_error(format!(
                    "unknown module member: {}",
                    q.parts.iter().map(|p| p.name.as_str()).collect::<Vec<_>>().join(".")
                )))
            }
            Expr::Postfix(pf) => {
                // x# → len(x), x~ → reversed(x), x^ → sorted(x), x_ → sorted_desc(x)
                let operand = self.gen_expr(&pf.operand, vars)?;
                ...
                self.gen_builtin_call(builtin_name, vec![operand], vars)
            }
            Expr::AssignExpr(a) => {
                // Assignment as expression — evaluate the assignment, return the value.
                // For now, just evaluate the value expression.
                self.gen_expr(&a.value, vars)
            }
            Expr::Spread(s) => {
                // Spread in expression context — just evaluate inner.
                self.gen_expr(&s.expr, vars)
            }
            Expr::OptionalChain(oc) => {
                // Optional chaining — evaluate the chain, return null if any link is null.
                // Simplified: just evaluate the first expression.
                self.gen_expr(&oc.target, vars)
            }
            Expr::TryPropagate(tp) => {
                // try-propagate: evaluate the expression, return it.
                self.gen_expr(&tp.expr, vars)
            }
