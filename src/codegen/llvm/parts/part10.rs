impl FullLlvmGen {
    ...
    fn collect_strings_expr(&mut self, e: &Expr) -> Result<()> {
        match e {
            ...
            // NullCoalesce sub-tree was previously skipped by the catch-all,
            // which meant string literals on either side of `??` were never
            // interned and gen_string_literal would fail at codegen time.
            // Traverse both operands explicitly.
            Expr::NullCoalesce(n) => {
                self.collect_strings_expr(&n.left)?;
                self.collect_strings_expr(&n.right)?;
            }
            _ => {}
        }
        Ok(())
