...
pub fn parse_isr_group_config(ann: &Annotation) -> IsrGroupConfig {
    ...
    for arg in &ann.arguments {
        if arg.name.as_deref() == Some("budget_us") {
            ...
            // Priority can be an integer or a string ("high"/"low").
            if let Expr::Integer(i) = &arg.value {
                config.priority = Some(i.value as u32);
            } else if let Expr::String_(s) = &arg.value {
                if let Some(StringPart::Text(t)) = s.parts.first() {
                    config.priority = match t.as_str() {
                        "high" => Some(0),
                        "medium" => Some(1),
                        "low" => Some(2),
                        _ => None,
        ...
        // "vector" parameter is handled by the firmware builder (emitter.rs)
        // to place the handler at the correct IRQ index. It's not an error.
    }
    config
}
