...
impl LinearChecker {
    ...
    fn check_program(&mut self, program: &PirProgram) -> Result<()> {
        ...
        for function in &program.functions {
            ...
            // 注册 ptr 参数为 MustConsume 资源。
            for (name, ty) in &function.params {
                if ty.is_ptr() {
                    fn_checker.resources.insert(name.clone(), LinearResource {
                        ty: ty.clone(),
                        state: LinearState::MustConsume,
                        origin: format!("ptr param in {}", function.name),
                    });
                }
            }
    ...
    fn check_node(&mut self, node: &PirNode) -> Result<()> {
        match node {
            ...
            | PirNode::Comment(_)
            | PirNode::BinOp { .. } => {
                // Physical metadata and permissive-lowering nodes do not
                // create or consume linear resources.
            }
        }
        Ok(())
    }
