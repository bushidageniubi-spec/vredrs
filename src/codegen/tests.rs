...
//
// All tests run through the bytecode VM (no AST interpreter fallback).
// The test helper `run_code` compiles and executes source code via the
// VM, then returns a `VmRunner` that provides the same `get()` interface
// as the old `Interpreter`.

use crate::bytecode::{Compiler, VM, Value as VmValue};
...
/// A wrapper around the VM's globals that provides the same `get()`
/// interface as `Interpreter`.
pub struct VmRunner {
    globals: HashMap<String, VmValue>,
}

impl VmRunner {
    /// Look up a variable by name and convert to interpreter Value.
    pub fn get(&self, name: &str) -> Value {
        match self.globals.get(name) {
            Some(v) => vm_to_interp_value(v),
            None => Value::Null,
        }
    }
}

/// Convert a VM Value to an interpreter Value.
fn vm_to_interp_value(v: &VmValue) -> Value {
    match v {
        VmValue::Int(i) => Value::Int(*i),
        VmValue::Float(f) => Value::Float(*f),
        VmValue::Bool(b) => Value::Bool(*b),
        VmValue::Str(s) => Value::Str(s.clone()),
        VmValue::Null => Value::Null,
        VmValue::List(l) => {
            Value::List(l.iter().map(vm_to_interp_value).collect())
        }
        VmValue::Tuple(t) => {
            Value::Tuple(t.iter().map(vm_to_interp_value).collect())
        }
        VmValue::Dict(d) => {
            let mut map = std::collections::HashMap::new();
            for (k, v) in d {
                map.insert(k.clone(), vm_to_interp_value(v));
            }
            Value::Dict(map)
        }
        VmValue::Func(name) => Value::Function(name.clone(), vec![], vec![], vec![]),
        VmValue::Generator(_) => Value::Null,
        VmValue::Object(class, fields) => {
            let mut f = std::collections::HashMap::new();
            for (k, v) in fields.borrow().iter() {
                f.insert(k.clone(), vm_to_interp_value(v));
            }
            Value::Object(class.clone(), Rc::new(RefCell::new(f)))
        }
        VmValue::Class(_) => Value::Null,
        VmValue::Module(_, _) => Value::Null,
        VmValue::Exception(msg) => Value::Str(msg.clone()),
    }
}
