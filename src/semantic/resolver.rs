...
impl Resolver {
    ...
    fn resolve_fn_def(&mut self, fd: &FnDef) -> Result<()> {
        ...
        let params: Vec<_> = fd
            ...
            .collect();
        let ret = fd.return_type.as_ref().map(|te| convert_type(te));
        self.symtab.declare(
            &fd.name.name,
            SymbolEntry::Function {
        ...
        // Declare generic type parameters (e.g. `T` in `fn, f[T](x: T)`)
        // as local type symbols so that type annotations referencing them
        // resolve without "unknown type" errors. In VM mode these are
        // treated as `Any`; in LLVM/Raw mode the monomorphizer substitutes
        // them with concrete types before codegen.
        for tp in &fd.type_params {
            let _ = self.symtab.declare(
                tp,
                SymbolEntry::Type {
                    name: tp.clone(),
                    info: TypeInfo::TypeVariable(tp.clone()),
                    span: Span::dummy(),
                },
            );
        }
    ...
    fn resolve_expr(&mut self, e: &Expr) -> Result<()> {
        match e {
            Expr::Identifier(id) => {
                // Don't report undefined variables — the resolver doesn't
                // know about imported symbols (resolved at runtime) or
                // builtins. Just record the lookup for potential future use.
