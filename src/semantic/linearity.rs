...
impl LinearityChecker {
    ...
    pub fn check(&self, prog: &Program) -> Result<()> {
        let mut checker = LinearityChecker::new();
        checker.check_program(prog);
        // Warnings are stored but not returned as errors in 0.1.1.
        // A future release can surface them via the LSP / CLI.
        let _ = checker.warnings;
        Ok(())
    }
