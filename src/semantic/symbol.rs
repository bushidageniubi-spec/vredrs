...
impl SymbolTable {
    ...
    pub fn declare(&mut self, name: &str, sym: SymbolEntry) -> Result<(), CompilerError> {
        ...
        // For Variable symbols, update the existing entry (reassignment)
        // instead of returning an error. This ensures the symbol table's
        // span/metadata reflects the most recent assignment.
        if scope.symbols.contains_key(name) {
            if matches!(sym, SymbolEntry::Variable { .. }) {
...
impl TypeInfo {
    ...
    /// Any/Unknown are compatible with everything. Same types are compatible.
    pub fn is_compatible_with(&self, other: &TypeInfo) -> bool {
        match (self, other) {
            ...
            // Type variables (generic type parameters) are compatible with
            // any type — the actual type is determined during monomorphization.
            (TypeInfo::TypeVariable(_), _) | (_, TypeInfo::TypeVariable(_)) => true,
            // Void is compatible only with Void. This matters for return-
            // type checking: a `: void` function with a bare `return;` is
            // a valid Void→Void return, while a `: int` function with a
            // bare `return;` (Void) is a mismatch. Without this arm the
            // match fell through to `_ => false`, which incorrectly
            // rejected Void-vs-Void as a mismatch.
            (TypeInfo::Void, TypeInfo::Void) => true,
            (TypeInfo::Basic(a), TypeInfo::Basic(b)) => a == b,
            (TypeInfo::Named(a), TypeInfo::Named(b)) => a == b,
            (TypeInfo::List(a), TypeInfo::List(b)) => a.is_compatible_with(b),
            (TypeInfo::Optional(a), Optional(b)) | (Optional(a), Optional(b)) => {
